...
import { getExpenseOverview } from "@/lib/aggregation";
...
export async function GET(req: NextRequest) {
  ...
  const db = getDb();
  return NextResponse.json(getExpenseOverview(db, org));
}

// Upserts the editable forecast (year1-3, responsible, note) for one category.
// year_current is never accepted from the client — it is always derived from 2.1-2.5.
export async function PUT(req: NextRequest) {
  ...
  db.prepare(
    `INSERT INTO expense_yearly_overview (id, organization_name, category, forecast_year1, forecast_year2, forecast_year3, responsible, note)
  ...
  const rows = getExpenseOverview(db, organization_name);
